...
import (
	...
	"github.com/zerjioang/gotools/lib/slice"
)
	...
	h.modifyHandles(func() {
		...
		for _, stream := range streams {
			if len(stream.pending) > 0 {
				...
				slice.OrderedDelete(&stream.pending, 0)
...
func (h *Handle) Cancel(t *Timer) {
	h.modify(func() {
		for i, timer := range h.timers {
			if timer == t {
				slice.UnorderedDelete(&h.timers, i)
			}
		}
	})
}
	...
	go func() {
		...
		e.modifyHandles(func() {
			for i, handle := range e.handles {
				if handle == h {
					slice.UnorderedDelete(&e.handles, i)
					return
				}
			}
...
func (e *EventLoop) step() (bool, error) {
	...
	for len(e.timers) > 0 {
		...
		slice.UnorderedDelete(&e.timers, minTimerIdx)
