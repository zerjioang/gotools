...
func TestChaining(t *testing.T) {
	testChaining(t, NewSlabArena(1, 1, 2, nil))
	testChaining(t, NewSlabArena(1, 100, 2, nil))
}

func testChaining(t *testing.T, s *SlabArena) {
	a := s.Alloc(1)
	f := s.Alloc(1)
...
func BenchmarkAllocingSize1(b *testing.B) {
	benchmarkAllocingConstant(b, NewSlabArena(1, 1024, 2, nil), 1)
}

func BenchmarkAllocingSize128(b *testing.B) {
	benchmarkAllocingConstant(b, NewSlabArena(1, 1024, 2, nil), 128)
}

func BenchmarkAllocingSize256(b *testing.B) {
	benchmarkAllocingConstant(b, NewSlabArena(1, 1024, 2, nil), 256)
}

func benchmarkAllocingConstant(b *testing.B, a *SlabArena, allocSize int) {
	stuff := [][]byte{}
	for i := 0; i < 1024; i++ {
...
func BenchmarkAllocingModSizes(b *testing.B) {
	benchmarkAllocingFunc(b, NewSlabArena(1, 1024, 2, nil),
		func(i int) int { return i % 1024 })
}

func BenchmarkAllocingModSizesGrowthFactor1Dot1(b *testing.B) {
	benchmarkAllocingFunc(b, NewSlabArena(1, 1024, 1.1, nil),
		func(i int) int { return i % 1024 })
}

func benchmarkAllocingFunc(b *testing.B, a *SlabArena,
...
func TestChainingSizes(t *testing.T) {
	testChainingSizes(t, NewSlabArena(1, 100, 2, nil))
	testChainingSizes(t, NewSlabArena(1, 200, 8, nil))
}

func testChainingSizes(t *testing.T, s *SlabArena) {
	curr := s.Alloc(91)
	for i := 90; i > 0; i-- {
...
func TestStats(t *testing.T) {
	a := NewSlabArena(1, 1024*1024, 2, nil)
	a.Alloc(3)
	aa := a.Alloc(17)
	if len(aa) != 17 {
