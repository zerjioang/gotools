...
// An opaque reference to bytes managed by an SlabArena.  See
// SlabArena.BufToLoc/LocToBuf().  A Loc struct is GC friendly in that a
// Loc does not have direct pointer fields into the SlabArena's memory
...
// NewSlabArena returns an SlabArena to manage byte slice memory based on a
// slab allocator approach.
//
// The startChunkSize and slabSize should be > 0.
// The growthFactor should be > 1.0.
// The malloc() func is invoked when SlabArena needs memory for a new slab.
// When malloc() is nil, then SlabArena defaults to make([]byte, size).
...
// an Alloc() from the same SlabArena.
...
// from an Alloc() from the same SlabArena.  Once the buf's ref-count
// drops to 0, the SlabArena may reuse the buf.  Returns true if this was
...
// buf's managed by an SlabArena can be chained.  The returned bufNext may
...
// bufNext.  The buf's from an SlabArena can be chained, where buf will
...
// BufToLoc returns a Loc that represents an SlabArena-managed buf.  Does
...
// LocToBuf returns a buf for an SlabArena-managed Loc.  Does not affect
...
func (s *SlabArena) allocChunk(bufLen int) (*slabClass, *chunk) {
	s.totAllocs++
...
func (s *SlabArena) findSlabClassIndex(bufLen int) int {
...
func (s *SlabArena) chunk(cl Loc) (*slabClass, *chunk) {
	if cl.IsNil() {
		return nil, nil
	}
...
// Determine the slabClass & chunk for an SlabArena managed buf []byte.
func (s *SlabArena) bufChunk(buf []byte) (*slabClass, *chunk) {
...
func (s *SlabArena) decRef(sc *slabClass, c *chunk) bool {
	c.refs--
	if c.refs < 0 {
