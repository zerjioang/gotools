...
var (
	...
	defaultValue = []byte{0x00, 0x00, 0x00, 0x00}
)
...
func NewMasterKey(seed []byte, key string, h func() hash.Hash) (*Key, error) {
	...
	intermediary := hmacSecret.Sum(nil)
	...
	genKey := &Key{
		...
		Key:         keyBytes,
		Depth:       0x0,
		ChildNumber: defaultValue,
		FingerPrint: defaultValue,
		IsPrivate:   true,
	}
...
func (key Key) getIntermediary(childIdx uint32) ([]byte, error) {
	...
	hmacSecret := hmac.New(sha512.New, key.ChainCode)
	_, err := hmacSecret.Write(data)
	if err != nil {
		return nil, err
	}
	return hmacSecret.Sum(nil), nil
}
