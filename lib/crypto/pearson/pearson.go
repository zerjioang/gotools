...
func (h PearsonHasher) Pearson8(x []byte) byte {
	...
	hash = byte(len(x) % 256)
	for i := 0; i < len(x); i++ {
		hash = lookupTable[int(hash^byte(i%256))]
	}
	return hash
}
