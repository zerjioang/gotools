// Package base64 implements base64 encoding as specified by RFC 4648.
package base64

var (
	charsetEncode = []byte{0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x4A, 0x4B, 0x4C, 0x4D, 0x4E, 0x4F, 0x50, 0x51, 0x52, 0x53, 0x54, 0x55, 0x56, 0x57, 0x58, 0x59, 0x5A, 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69, 0x6A, 0x6B, 0x6C, 0x6D, 0x6E, 0x6F, 0x70, 0x71, 0x72, 0x73, 0x74, 0x75, 0x76, 0x77, 0x78, 0x79, 0x7A, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x2B, 0x2F}
	decodeMapArr  = []byte{0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x3E, 0xFF, 0xFF, 0xFF, 0x3F, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x3A, 0x3B, 0x3C, 0x3D, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x0, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xA, 0xB, 0xC, 0xD, 0xE, 0xF, }
	padByte       = byte('=')
)

func EncodeFast(src []byte) []byte {
	// calculate the final lenght of the string
	ls := len(src)
	encodeLen := (ls + 2) / 3 * 4 // minimum # 4-char quanta, 3 bytes each
	// prepare a buffer for final string
	dst := make([]byte, encodeLen)
	// Divid the input bytes stream into blocks of 3 bytes.
	blocks := ls / 3
	// Divid 24 bits of each 3-byte block into 4 groups of 6 bits.
	di, si := 0, 0
	n := blocks * 3
	for si < n {
		// Convert 3x 8bit source bytes into 4 bytes
		val := uint(src[si+0])<<16 | uint(src[si+1])<<8 | uint(src[si+2])
		dst[di+0] = charsetEncode[val>>18&0x3F]
		dst[di+1] = charsetEncode[val>>12&0x3F]
		dst[di+2] = charsetEncode[val>>6&0x3F]
		dst[di+3] = charsetEncode[val&0x3F]
		si += 3
		di += 4
	}
	remain := ls - si
	if remain == 0 {
		return dst
	}
	// Add the remaining small block
	val := uint(src[si+0]) << 16
	if remain == 2 {
		val |= uint(src[si+1]) << 8
	}
	dst[di+0] = charsetEncode[val>>18&0x3F]
	dst[di+1] = charsetEncode[val>>12&0x3F]
	// Map each group of 6 bits to 1 printable character
	// based on the 6-bit value using the Base64 character set map.
	switch remain {

	case 2:
		dst[di+2] = charsetEncode[val>>6&0x3F]
		dst[di+3] = padByte
	case 1:
		dst[di+2] = padByte
		dst[di+3] = padByte
	}
	return dst
}
