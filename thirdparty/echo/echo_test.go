...
func TestEchoFile(t *testing.T) {
	...
	assert.Equal(t, codes.StatusOK, c)
	assert.NotEmpty(t, b)
}
...
func TestEchoMiddleware(t *testing.T) {
	...
	e.GET("/", func(c Context) error {
		return c.String(codes.StatusOK, "OK")
	})
...
func TestEchoMiddlewareError(t *testing.T) {
	...
	assert.Equal(t, codes.StatusInternalServerError, c)
}
...
func TestEchoWrapHandler(t *testing.T) {
	...
	h := WrapHandler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(codes.StatusOK.Int())
		w.Write([]byte("test"))
	}))
	if assert.NoError(t, h(c)) {
		assert.Equal(t, codes.StatusOK, rec.Code)
...
func TestEchoAny(t *testing.T) { // JFC
	...
	e.Any("/", func(c Context) error {
		return c.String(codes.StatusOK, "Any")
	})
}
...
func TestEchoMatch(t *testing.T) { // JFC
	...
	e.Match([]string{http.MethodGet, http.MethodPost}, "/", func(c Context) error {
		return c.String(codes.StatusOK, "Match")
	})
}
...
func TestEchoEncodedPath(t *testing.T) {
	...
	e.GET("/:id", func(c Context) error {
		return c.NoContent(codes.StatusOK)
	})
...
func TestEchoNotFound(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	e.ServeHTTP(rec, req)
	assert.Equal(t, codes.StatusNotFound, rec.Code)
}

func TestEchoMethodNotAllowed(t *testing.T) {
	e := New()
	e.GET("/", func(c Context) error {
		return c.String(codes.StatusOK, "Echo!")
	})
	req := httptest.NewRequest(http.MethodPost, "/", nil)
	rec := httptest.NewRecorder()
	e.ServeHTTP(rec, req)
	assert.Equal(t, codes.StatusMethodNotAllowed, rec.Code)
}

func TestEchoContext(t *testing.T) {
	e := New()
	c := e.AcquireContext()
	assert.IsType(t, new(HttpContext), c)
	e.ReleaseContext(c)
}
...
func testMethod(t *testing.T, method, path string, e *Echo) {
	...
	h := reflect.ValueOf(func(c Context) error {
		return c.String(codes.StatusOK, method)
	})
	i := interface{}(e)
...
func TestHTTPError(t *testing.T) {
	err := NewHTTPError(codes.StatusBadRequest, map[string]interface{}{
		"code": 12,
	})
